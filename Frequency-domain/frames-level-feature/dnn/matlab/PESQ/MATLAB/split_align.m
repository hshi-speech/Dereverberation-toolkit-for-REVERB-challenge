...
% fprintf( 'Utt_DelayEst_l, Utt_Start_l, N_BPs is %d,%d,%dn', ...
...
%     fprintf( 'bp,Utt_BPs(%d) is %d,%dn', bp,bp,Utt_BPs(bp)); 
...
% for count= 1: N_BPs- 1 
%     fprintf( stream, '%dn', Utt_ED2(count));
% end
% fclose( stream );
...
while( 1 )
    ...
    estdelay = Utt_ED1(bp);
%     fprintf( 'bp,estdelay is %d,%dn', bp, estdelay);
    H(1: Align_Nfft)= 0;
    Hsum = 0.0;
    
    startr = (Utt_Start_l- 1) * Downsample+ 1;
    startd = startr + estdelay;
%     fprintf( 'startr/startd is %d/%dn', startr, startd);
...
%         fprintf( stream, '%f %fn', v_max, n_max);
    ...
    if( Hsum > 0.0 )
%         if (Utt_Len== 236)
%             fprintf( 'v_max, Hsum is %f, %fn', v_max, Hsum);
%         end
        Utt_DC1(bp) = v_max / Hsum;
    else
        Utt_DC1(bp) = 0.0;
    end
%     fprintf( 'bp/startr/startd is %d/%d/%dn', bp, startr, startd);
    while( bp < (N_BPs - 1) )
        bp = bp + 1;
...
% %                 end
                ...
                v_max = 0.99* max( X1);
                n_max = (v_max^ 0.125)/ kernel;
%                 fprintf( 'v_max n_max is %f %fn', v_max, n_max);
...
%     fprintf( '***NEW startr is %dn', startr);
    
%     fprintf( 'startr/d, deg_Nsamples is %d/%d, %dn', startr,startd, ...
%         deg_Nsamples);
%     fprintf( 'deg_data has %d elementsn', numel( deg_data));
...
% fprintf( fid, '%fn', Utt_D2);
% fprintf( fid, 'n');
% fprintf( fid, '%fn', Utt_DC2);
% fclose( fid);
% fprintf( 'Utt_Len, N_BPs is %d, %dn', Utt_Len, N_BPs);
for bp = 1: N_BPs- 1
...
%     fprintf( fid, 'N_BPs is %dn', N_BPs);
%     fprintf( fid, 'Utt_DelayConf is %fn', Utt_DelayConf_l);
%     fprintf( fid, 'ED2t ED1t D2t D1t DC2t DC1t BPsn');
%     for bp= 1: N_BPs- 1
%         fprintf( fid, '%dt %dt %dt %dt %ft %ft %dn', Utt_ED2( bp), ...
